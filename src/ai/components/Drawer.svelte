...
<AppsPopup {isDark} open={appsPopupOpen} currentAppId="ai" on:close={() => appsPopupOpen = false} />
...
<div class="drawer" class:open class:dark={isDark}>
  ...
  <div class="header">
    <span class="header-title" style="color:{c.textPrimary}">Menu</span>
    ...
    <button type="button" class="user-footer" class:dark={isDark} on:click={openSettings}>
      ...
      <div class="user-info">
        ...
        {#if userEmail && userEmail !== userName}
          <span class="user-email" style="color:{c.textSecondary}">{userEmail}</span>
        {/if}
      </div>
    </button>
  {/if}
...
<style>
  ...
  .drawer {
    ...
    width: 78vw; max-width: 320px; z-index: 101;
  ...
  .section-label { font-size: 11.5px; font-weight: 600; letter-spacing: 0.06em; text-transform: uppercase; color: rgba(60,60,67,0.5); padding: 20px 20px 6px; }
  .dark .section-label { color: rgba(235,235,245,0.4); }
  .section { display: flex; flex-direction: column; padding: 0 12px; }

  .row { width: 100%; display: flex; align-items: center; gap: 13px; padding: 13px 10px; background: transparent; border: none; cursor: pointer; text-align: left; border-radius: 10px; transition: background .12s ease; color: #000; font-family: -apple-system, BlinkMacSystemFont, sans-serif; }
  .dark .row { color: #fff; }
  ...
  .row-chevron { width: 13px; height: 13px; background: rgba(60,60,67,0.28); transition: transform .25s cubic-bezier(0.4,0,0.2,1); flex-shrink: 0; }

<script>
  ...
  async function handleRegister() {
    ...
    try {
      ...
      } else { errorMsg = 'Não foi possível criar a conta. Tenta com outro email.'; }
...
<div class="auth-page" style="background:{isDark ? '#0C0C0F' : '#FFFFFF'}">
  ...
  <div class="auth-inner">
    <div class="auth-logo-wrap">
      ...
      <div class="auth-title" class:dark={isDark}>Cria a tua<br><span>conta</span> agora.</div>
      <div class="auth-subtitle" class:dark={isDark}>É rápido e gratuito</div>
    </div>
    <div class="auth-field">
      <input class="auth-input" class:dark={isDark} type="text" placeholder="Nome completo" autocomplete="name" bind:value={name} />
    </div>
    <div class="auth-field">
    ...
    <div class="auth-divider" class:dark={isDark}><hr /><span>ou continua com</span><hr /></div>
    <button class="auth-btn-google" class:dark={isDark} on:click={handleGoogle}>
      <img src="/icons/png/google.png" alt="Google" /> Google
    </button>
    <div class="auth-switch" class:dark={isDark}>
      Já tens conta? <span class="auth-switch-link" on:click={() => dispatch('nav', { to: 'login' })}>Iniciar sessão</span>
    </div>
  </div>
</div>

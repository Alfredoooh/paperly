<script>
  ...
  let hue = 210, satPct = 70, brightPct = 90;
  ...
  function confirmColor() { dispatch('confirm', previewHex); }
  ...
  function onTouchEnd() {
    ...
    if (draggedFraction > 0.3 || (delta > 0 && velocity > 0.5)) cancel();
  ...
  <div class="bottom-sheet" bind:this={sheetEl} style="background:{c.dialogBackground};transform:translate3d(0,{sheetY}%,0);">
    ...
    <div class="sheet-body">
      ...
      <div class="sheet-actions">
        ...
        <button class="btn-primary" on:click={confirmColor}>Aplicar</button>
      </div>
    </div>
  </div>
{/if}
...
<style>
  ...
  .bottom-sheet { position:fixed; bottom:0; left:0; right:0; border-radius:20px 20px 0 0; z-index:700; padding:0 0 calc(env(safe-area-inset-bottom,0px) + 24px); will-change:transform; box-shadow:0 -4px 40px rgba(0,0,0,.16); }

  ...
  <div class="bottom-sheet sheet-tall" bind:this={sheetEl} style="background:{c.dialogBackground};transform:translate3d(0,{sheetY}%,0);">
    ...
    <div class="sheet-body">
      <div class="unit-switch" style="background:{c.appbarBtnBg}">
        <button class="unit-btn" style={sizeUnit === 'px' ? 'background:var(--accent-primary);color: var(--icon-strong)' : `color:${c.textPrimary}`} on:click={() => setSizeUnit('px')}>px</button>
        <button class="unit-btn" style={sizeUnit === 'cm' ? 'background:var(--accent-primary);color: var(--icon-strong)' : `color:${c.textPrimary}`} on:click={() => setSizeUnit('cm')}>cm</button>
      </div>
...
<style>
  ...
  .apply-btn { width:100%; margin-top:14px; border:none; border-radius:999px; padding:13px 16px; font-size:14px; font-weight:700; cursor:pointer; background:var(--accent-primary); color: var(--icon-strong); -webkit-tap-highlight-color:transparent; transition:transform .16s cubic-bezier(0.34,1.56,0.64,1); }
